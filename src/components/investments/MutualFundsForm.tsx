...
const formSchema = z.object({
  ...
  incomeDetails: z.string().min(1, "Please provide income details"),
  salaryDetails: z.string().min(1, "Please provide salary details"),
});

export type MutualFundsFormData = z.infer<typeof formSchema>;

interface MutualFundsFormProps {
  onBack: () => void;
}

export const MutualFundsForm = ({ onBack }: MutualFundsFormProps) => {
  const { user } = useAuth();
  const [showBanner, setShowBanner] = useState(false);
  const [guestId, setGuestId] = useState<string | null>(null);

  const form = useForm<MutualFundsFormData>({
  ...
  const onSubmit = async (data: MutualFundsFormData) => {
    console.log("Mutual Funds Form submitted:", data);
    if (!user) {
  ...
  const onSubmitted = (type) => {
    console.log("Mutual Funds Application: TruConsentModal onClose triggered with type:", type);
      ...
      <CardHeader>
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <CardDescription>
              Powered by mars.money • Start investing in diversified funds
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-6">
              <h3 className="text-xl font-semibold text-gray-900 border-b pb-3">Income & Salary</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField
                  control={form.control}
                  name="incomeDetails"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Income Details *</FormLabel>
                      <FormControl>
                        <Input placeholder="Annual income, other income etc." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <div className="flex gap-4 pt-6">
              ...
              <Button type="submit" className="flex-1 bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700" size="lg">
                Continue
              </Button>
            </div>
          </form>
